...
func (c *memoServiceClient) SearchWithHighlight(ctx context.Context, in *SearchWithHighlightRequest, opts ...grpc.CallOption) (*SearchWithHighlightResponse, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(SearchWithHighlightResponse)
	err := c.cc.Invoke(ctx, MemoService_SearchWithHighlight_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _MemoService_SearchWithHighlight_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(SearchWithHighlightRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(MemoServiceServer).SearchWithHighlight(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: MemoService_SearchWithHighlight_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(MemoServiceServer).SearchWithHighlight(ctx, req.(*SearchWithHighlightRequest))
	}
	return interceptor(ctx, in, info, handler)
}
