# AI Chat 界面架构 - 差距分析与修复方案

> **创建日期**: 2026-02-06
> **版本**: v1.0
> **关联**: [AI Chat Interface Architecture](../../dev-guides/AI_CHAT_INTERFACE.md) | [Unified Block Model](../unified-block-model.md)

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 执行摘要

本文档基于对 DivineSense AI Chat 界面的深度代码审查，识别设计文档与实际实现之间的差距，并提供 AI Native 原则下的产品设计方案和实施计划。

**核心发现**：
1. 后端 Unified Block Model 已完整实现并持久化数据
2. 前端组件部分实现，但存在数据流断裂
3. 文档与实现存在多处不一致
4. UI 层级和元素分配需要重新平衡

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 1. 现状分析

### 1.1 架构概览

```
┌─────────────────────────────────────────────────────────────────┐
│                      当前实现状态                               │
├─────────────────────────────────────────────────────────────────┤
│  层级              │ 组件                  │ 状态  │ 备注       │
├─────────────────────────────────────────────────────────────────┤
│  后端数据模型     │ ai_block 表          │ ✅    │ 完整实现   │
│                   │ event_stream (JSONB)  │ ✅    │ 持久化     │
│                   │ session_stats (JSONB) │ ✅    │ 持久化     │
│                   │ token_usage (JSONB)   │ ✅    │ 持久化     │
├─────────────────────────────────────────────────────────────────┤
│  后端服务层      │ BlockManager          │ ✅    │ Create/     │
│                   │                       │       │ Update/     │
│                   │                       │       │ AppendEvent │
├─────────────────────────────────────────────────────────────────┤
│  后端 API        │ ListBlocks/GetBlock   │ ✅    │ gRPC       │
│                   │ CreateBlock           │ ✅    │            │
│                   │ UpdateBlock           │ ✅    │            │
│                   │ AppendEvent           │ ✅    │            │
├─────────────────────────────────────────────────────────────────┤
│  前端数据层      │ useBlockQueries        │ ✅    │ React Query │
│                   │ useBlocksWithFallback  │ ✅    │ Fallback    │
├─────────────────────────────────────────────────────────────────┤
│  前端页面层      │ AIChat.tsx             │ ✅    │ 主入口     │
│                   │ ChatMessages.tsx       │ ✅    │ 消息列表   │
│                   │ ChatInput.tsx          │ ✅    │ 输入区域   │
│                   │ ChatHeader.tsx         │ ✅    │ 状态头部   │
├─────────────────────────────────────────────────────────────────┤
│  前端组件层      │ UnifiedMessageBlock    │ ⚠️    │ 部分实现   │
│                   │   - BlockHeader        │ ⚠️    │ 缺失元素   │
│                   │   - BlockBody          │ ✅    │ 完整       │
│                   │   - BlockFooter        │ ⚠️    │ 缺失操作   │
│                   │ BranchIndicator        │ ⚠️    │ 部分集成   │
│                   │ ExpandedSessionSummary │ ✅    │ Geek/Evo   │
└─────────────────────────────────────────────────────────────────┘
```

### 1.2 数据流分析

```
用户输入 → ChatInput.tsx
    │
    ▼
AIChat.tsx (handleSend)
    │
    ├──► 创建/选择会话
    │
    ├──► 添加用户消息到 Context
    │
    └──► chatHook.stream()
            │
            ├── onThinking → updateMessage() ───► ❌ 仅存储在内存 Context
            ├── onToolUse   → updateMessage() ───► ❌ 仅存储在内存 Context
            ├── onToolResult → updateMessage() ───► ❌ 仅存储在内存 Context
            ├── onContent   → updateMessage() ───► ❌ 仅存储在内存 Context
            └── onDone      → refetchBlocks() ───► ✅ 从数据库获取完整 Block
                                                            │
                                                            ▼
                                                    ChatMessages.tsx
                                                    convertAIBlocksToMessageBlocks()
                                                            │
                                                            ▼
                                                    extractToolCalls(eventStream) ──► ✅ 从持久化数据提取
                                                    extractThinkingSteps(eventStream)
                                                            │
                                                            ▼
                                                    UnifiedMessageBlock
                                                    显示完整内容
```

**关键发现**：
- 流式期间：数据仅存储在内存 Context（刷新后丢失）
- 完成后：`refetchBlocks()` 从数据库获取完整 Block
- `event_stream` 持久化完整，但前端主要依赖内存 Context 渲染

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 2. 差距清单

### 2.1 文档与实现不一致

| # | 文档描述 | 实际实现 | 影响 | 优先级 |
|:--|:---------|:---------|:-----|:-------|
| 1 | Mode Switcher 在 HEADER 区域 | Mode Switcher 在 INPUT AREA (ChatInput) | 文档误导 | P3 |
| 2 | Header 显示 Session Info | 无此组件 | 缺失功能 | P2 |
| 3 | Header 显示当前 Mode（冗余） | 无此冗余设计 | 设计优化 | P3 |
| 4 | Block Header 完整实现 | 部分实现（见下表） | 功能缺失 | P1 |
| 5 | Tool Calls 刷新后消失 | 实际已持久化（convertAIBlocksToMessageBlocks 正确提取） | 已修复 | - |
| 6 | "Session Summary" | 应为 "Block Summary" | 命名混淆 | P2 |

### 2.2 Block Header 实现差距

| 元素 | 设计要求 | 当前实现 | 缺失原因 | 优先级 |
|:-----|:---------|:---------|:---------|:-------|
| 🌿 Branch Indicator | 显示分支路径（0/1/2） | BranchIndicator 组件存在但未显示完整数据 | 数据未从 Block 读取 | P1 |
| 🤖 Parrot Routing | AUTO → MemoParrot | 无此显示 | 需要路由决策记录 | P2 |
| ⏱ Duration | 显示处理时间（2.3s） | 无 | sessionStats 未在 Block Header 显示 | P1 |
| 💰 Cost | 显示成本（$0.0087） | 仅 Geek/Evolution 模式 | Normal 模式无成本估算 | P1 |
| 🔄 Regenerate | 重新生成按钮 | 有 | - | - |
| 📋 Copy | 复制按钮 | 有 | - | - |
| ✏️ Edit | 编辑用户输入按钮 | **无** | 未实现 | P0 |
| 🗑️ Delete | 删除 Block 按钮 | 有（仅最新） | - | - |

### 2.3 数据持久化差距

| 数据 | 后端存储 | 前端读取 | 前端显示 | 状态 |
|:-----|:---------|:---------|:---------|:-----|
| user_inputs | ✅ user_inputs JSONB | ✅ Block.userInputs | ✅ UserInputsSection | 完成 |
| assistant_content | ✅ assistant_content | ✅ Block.assistantContent | ✅ AnswerSection | 完成 |
| event_stream | ✅ event_stream JSONB | ✅ Block.eventStream | ✅ ToolCallsSection | 完成 |
| thinking_steps | ✅ event_stream (type: thinking) | ✅ extractThinkingSteps() | ✅ ThinkingSection | 完成 |
| token_usage | ✅ token_usage JSONB | ✅ Block.tokenUsage | ❌ 未显示 | 缺失 |
| session_stats | ✅ session_stats JSONB | ✅ Block.sessionStats | ⚠️ 仅 Geek/Evolution | 部分 |
| cost_estimate | ❌ 无独立字段 | ❌ 无 | ⚠️ 从 sessionStats 计算 | 缺失 |

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 3. AI Native 产品设计

### 3.1 设计原则

1. **数据单一真相源（Single Source of Truth）**
   - Block 是唯一真相源
   - 所有 UI 元素从 Block 数据派生
   - 流式期间乐观更新，完成后从数据库刷新

2. **渐进式披露（Progressive Disclosure）**
   - 默认折叠历史 Block
   - 最新 Block 默认展开
   - 详细统计默认隐藏，点击展开

3. **移动优先（Mobile First）**
   - 响应式设计，移动端优先
   - 触摸友好的交互
   - 关键操作始终可见

4. **状态可视化（State Visualization）**
   - 清晰的状态指示（thinking、tools、answer）
   - 错误状态突出显示
   - 进度实时反馈

### 3.2 界面层级重新设计

```
┌─────────────────────────────────────────────────────────────────┐
│                    AI Chat 界面架构 v2.0                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  OPTIONAL: Session Bar (会话汇总栏 - 可折叠)              │   │
│  │  ┌─────────────┬─────────────┬─────────────┐              │   │
│  │  │ Session: #123 │ Cost: $0.23 │ Tokens: 15K │              │   │
│  │  └─────────────┴─────────────┴─────────────┘              │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  ChatHeader (状态头部 - 桌面端显示)                       │   │
│  │  🤖 Assistant Name  │  💬 Thinking...                     │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  ChatMessages (消息区域)                                  │   │
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  UnifiedMessageBlock #1 (折叠状态)                  │   │   │
│  │  │  [👤 用户消息预览...] [14:30] [▼]                 │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  │                                                          │   │
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  UnifiedMessageBlock #2 (展开状态)                  │   │   │
│  │  │  ┌───────────────────────────────────────────┐   │   │   │
│  │  │  │  Block Header                              │   │   │   │
│  │  │  │  [👤] 搜索 Neo4j 图数据库                      │   │   │   │
│  │  │  │  ┌─────────────────────────────────────┐  │   │   │   │
│  │  │  │  │ 🌿 0/1 │ 🤖 AUTO │ ⏱ 2.3s │ 💰 $0.0087 │  │   │   │   │
│  │  │  │  └─────────────────────────────────────┘  │   │   │   │
│  │  │  │  [▼]                                             │   │   │   │
│  │  │  └───────────────────────────────────────────┘   │   │   │
│  │  │                                                      │   │   │
│  │  │  ┌───────────────────────────────────────────┐   │   │   │
│  │  │  │  Block Body (可折叠)                         │   │   │   │
│  │  │  │  ┌─ User Inputs ─────────────────────────┐  │   │   │   │
│  │  │  │  │ 👤 "搜索 Neo4j 图数据库"               │  │   │   │   │
│  │  │  │  └──────────────────────────────────────┘  │   │   │   │
│  │  │  │  ┌─ Thinking Process (可折叠) ──────────┐  │   │   │   │
│  │  │  │  │ 🧠 分析用户查询...                  │  │   │   │   │
│  │  │  │  └──────────────────────────────────────┘  │   │   │   │
│  │  │  │  ┌─ Tool Calls ─────────────────────────┐  │   │   │   │
│  │  │  │  │ 🔧 memo_search                         │  │   │   │   │
│  │  │  │  │    Query: "Neo4j 图数据库"            │  │   │   │   │
│  │  │  │  │    Results: 3 blocks                  │  │   │   │   │
│  │  │  │  └──────────────────────────────────────┘  │   │   │   │
│  │  │  │  ┌─ AI Answer ─────────────────────────┐  │   │   │   │
│  │  │  │  │ ⚡ 找到 3 条相关笔记...             │  │   │   │   │
│  │  │  │  └──────────────────────────────────────┘  │   │   │   │
│  │  │  │  ┌─ Block Summary (可折叠) ────────────┐  │   │   │   │
│  │  │  │  │ 📊 LLM Stats: 1,279 tokens, $0.0087 │  │   │   │   │
│  │  │  │  └──────────────────────────────────────┘  │   │   │   │
│  │  │  └───────────────────────────────────────────┘   │   │   │
│  │  │                                                      │   │   │
│  │  │  ┌───────────────────────────────────────────┐   │   │   │
│  │  │  │  Block Footer (操作栏)                      │   │   │   │
│  │  │  │  [▼ 折叠] [🔄 重新生成] [✏️ 编辑] [📋 复制] [🗑️] │   │   │
│  │  │  └───────────────────────────────────────────┘   │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  ChatInput (输入区域)                                   │   │
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  [⌘N] [✂️K] [🗑️L] ──────── ⌨️ Enter 发送 ────────── │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  [○ Normal ○ Geek ○ Evolution] │ [🔽 Mode▼]  │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  │ 输入消息...                                │   │   │
│  │  │  └─────────────────────────────────────────────┘   │   │
│  │  └───────────────────────────────────────────────────┘   │   │
│  │                                      [➤ Send]           │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

### 3.3 组件职责重新分配

#### 3.3.1 BlockHeader（紧凑型）

**职责**：快速识别 Block 内容和状态

| 元素 | 移动端 | 桌面端 | 实现优先级 |
|:-----|:-------|:-------|:-----------|
| 用户头像 + 消息预览 | ✅ | ✅ | P0 |
| 时间戳 | ✅ | ✅ | P0 |
| 折叠/展开按钮 | ✅ | ✅ | P0 |
| 状态徽章（thinking/error） | ✅ | ✅ | P0 |
| 分支指示器（如有分支） | ⚠️ 图标 | ✅ 完整 | P1 |
| **移除**：Parrot 名称 | - | - | - |
| **移除**：Session 统计（移到 Body） | - | - | - |

#### 3.3.2 BlockBody（主体）

**职责**：展示详细内容

| 子组件 | 描述 | 实现状态 | 优先级 |
|:-------|:-----|:---------|:-------|
| UserInputsSection | 用户输入（支持多个） | ✅ | P0 |
| ThinkingSection | 思考过程（可折叠） | ✅ | P0 |
| ToolCallsSection | 工具调用列表 | ✅ | P0 |
| AnswerSection | AI 回复内容 | ✅ | P0 |
| **BlockSummarySection** | Token/Cost/时间统计 | ⚠️ 仅 Geek | P1 |

#### 3.3.3 BlockFooter（操作栏）

**职责**：提供操作入口

| 操作 | 移动端 | 桌面端 | 实现状态 | 优先级 |
|:-----|:-------|:-------|:---------|:-------|
| 折叠/展开 | 文字 | 图标+文字 | ✅ | P0 |
| 重新生成 | 🔄 图标 | 图标+文字 | ✅ | P0 |
| 复制 | 📋 图标 | 图标+文字 | ✅ | P0 |
| **编辑** | ❌ | ❌ | ❌ | **P0** |
| 删除 | ❌ | ❌ | ⚠️ 仅最新 | P1 |
| **Fork** | ❌ | ❌ | ❌ | P2 |

### 3.4 新增组件设计

#### 3.4.1 SessionBar（会话汇总栏）

```
┌─────────────────────────────────────────────────────────────────┐
│  💬 Session: #12345  │  💰 $0.0234  │  🪙 15.2K tokens  │  ⏱ 2m  │
└─────────────────────────────────────────────────────────────────┘
```

**特性**：
- 可折叠（点击折叠/展开）
- 实时更新（流式完成后刷新）
- 移动端隐藏或简化显示

**数据来源**：聚合当前会话所有 Blocks
- 总成本 = Σ block.cost_estimate
- 总 Token = Σ block.token_usage.total_tokens
- 总时间 = Σ (最后更新 - 最先创建)

#### 3.4.2 BlockEditDialog（编辑对话框）

```
┌─────────────────────────────────────────────────────────────────┐
│  ✏️ 编辑用户输入                                                 │
├─────────────────────────────────────────────────────────────────┤
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  原始消息:                                             │   │
│  │  "搜索 Neo4j 图数据库笔记"                              │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  编辑后的消息:                                         │   │
│  │  ┌─────────────────────────────────────────────────┐  │   │
│  │  │ 搜索 Neo4j 图数据库和 Cypher 查询相关的笔记     │  │   │
│  │  └─────────────────────────────────────────────────┘  │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ⚠️ 注意：编辑用户输入将创建新的分支，原 Block 保持不变。      │
│                                                                  │
│  [取消]  [创建分支并重新生成]                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 4. 修复方案与实施计划

### 4.1 P0 优先级（必须修复）

#### 4.1.1 实现 Block 编辑功能

**目标**：允许用户编辑已发送的用户输入，创建新分支并重新生成

**API**：
- ForkBlock：已存在 (`proto/api/v1/ai_service.proto`)
- 需要前端实现 `ForkBlock` 调用

**前端实现**：
```typescript
// 新增组件: BlockEditDialog.tsx
interface BlockEditDialogProps {
  blockId: number;
  originalContent: string;
  onConfirm: (newContent: string) => void;
}
```

**实施步骤**：
1. 创建 `BlockEditDialog` 组件
2. 添加 `✏️ Edit` 按钮到 BlockFooter
3. 实现 `handleEdit` 函数，调用 `forkBlock` API
4. 更新 Block 后自动触发生成

**文件**：
- `web/src/components/AIChat/BlockEditDialog.tsx` (新建)
- `web/src/components/AIChat/UnifiedMessageBlock.tsx` (修改)
- `web/src/hooks/useBlockQueries.ts` (添加 useForkBlock)

#### 4.1.2 修复成本和 Token 统计显示

**问题**：Normal 模式下无成本和 Token 显示

**原因分析**：
- 后端仅在 Geek/Evolution 模式收集 `sessionStats`
- Normal 模式未调用 `UpdateBlockStatus`，导致 `session_stats` 为空

**修复方案**：
1. 后端：Normal 模式也收集基础 Token 统计
2. 前端：从 `token_usage` 字段读取并显示

**后端修改**：`server/router/api/v1/ai/handler.go`
```go
// 在流式完成后，为所有模式收集基础统计
func (h *Handler) finalizeBlock(ctx context.Context, blockID int64, content string, stats *SessionStats) {
    // 确保 token_usage 始终有值
    if stats != nil && stats.TotalTokens > 0 {
        // 转换为 TokenUsage
        tokenUsage := &store.TokenUsage{
            PromptTokens:     int32(stats.InputTokens),
            CompletionTokens: int32(stats.OutputTokens),
            TotalTokens:      int32(stats.TotalTokens),
            CacheReadTokens:  int32(stats.CacheReadTokens),
            CacheWriteTokens: int32(stats.CacheWriteTokens),
        }
        // 更新 Block
        h.blockManager.CompleteBlock(ctx, blockID, content, stats)
    }
}
```

**前端修改**：`UnifiedMessageBlock.tsx` - BlockHeader
```tsx
// 显示成本和 Token（所有模式）
{block.tokenUsage && (
  <div className="flex items-center gap-2 text-xs">
    {block.tokenUsage.totalTokens > 0 && (
      <span title="Total Tokens">
        🪙 {(block.tokenUsage.totalTokens / 1000).toFixed(1)}k
      </span>
    )}
  </div>
)}
```

### 4.2 P1 优先级（重要功能）

#### 4.2.1 完善 BlockHeader 信息显示

**目标**：显示分支路径、持续时间、成本

**实施步骤**：
1. 从 `Block.branchPath` 解析分支路径显示
2. 计算持续时间：`updated_ts - created_ts`
3. 计算成本：`cost_estimate / 100000`（从 milli-cents 转美元）

**代码位置**：`UnifiedMessageBlock.tsx` - BlockHeader

```tsx
// 分支路径显示
{block.branchPath && (
  <span className="flex items-center gap-1 text-xs" title="Branch path">
    🌿 {block.branchPath}
  </span>
)}

// 持续时间显示
{block.assistantTimestamp && block.createdTs && (
  <span className="flex items-center gap-1 text-xs" title="Duration">
    ⏱ {((block.assistantTimestamp - block.createdTs) / 1000).toFixed(1)}s
  </span>
)}

// 成本显示
{block.costEstimate && block.costEstimate > 0 && (
  <span className="flex items-center gap-1 text-xs text-green-600" title="Cost">
    💰 ${(block.costEstimate / 100000).toFixed(4)}
  </span>
)}
```

#### 4.2.2 实现 SessionBar（会话汇总栏）

**目标**：在页面顶部显示会话级汇总统计

**实施步骤**：
1. 创建 `SessionBar` 组件
2. 聚合当前会话所有 Blocks 的统计
3. 添加折叠/展开功能

**文件**：
- `web/src/components/AIChat/SessionBar.tsx` (新建)

**代码位置**：`AIChat.tsx` - 插入在 ChatHeader 和 ChatMessages 之间

#### 4.2.3 修复 BranchIndicator 显示

**问题**：BranchIndicator 组件存在但未完整显示分支信息

**修复**：
1. 确保 `branches` prop 正确传递
2. 从 Block 数据构建分支树
3. 实现分支选择对话框

**文件**：
- `web/src/components/AIChat/BranchIndicator.tsx` (修改)
- `web/src/components/AIChat/BranchSelectorDialog.tsx` (新建)

### 4.3 P2 优先级（优化改进）

#### 4.3.1 文档更新

**目标**：修正文档中的 ASCII 图，与实际实现一致

**修改文件**：
- `docs/dev-guides/AI_CHAT_INTERFACE.md`

**修改内容**：
1. Mode Switcher 位置：从 HEADER 移到 INPUT AREA
2. 移除不存在的 Session Info（或标注为待实现）
3. 更新 Block Header 组件图
4. 修正 "Session Summary" → "Block Summary"

#### 4.3.2 移动端优化

**目标**：确保所有功能在移动端可用

**优化点**：
1. Block Footer：隐藏文字，仅显示图标
2. BlockSummarySection：默认折叠
3. SessionBar：默认隐藏或简化显示

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 5. 实施计划

### 5.1 Phase 1：核心功能修复（1-2 周）

| 任务 | 文件 | 工作量 | 依赖 |
|:-----|:-----|:-------|:-----|
| 实现 Block 编辑 | BlockEditDialog.tsx | 2d | ForkBlock API |
| 修复 Token/Cost 显示 | handler.go, UnifiedMessageBlock.tsx | 2d | 后端统计 |
| 完善 BlockHeader 信息 | UnifiedMessageBlock.tsx | 1d | - |
| 更新文档 | AI_CHAT_INTERFACE.md | 1d | - |

### 5.2 Phase 2：会话级功能（1 周）

| 任务 | 文件 | 工作量 | 依赖 |
|:-----|:-----|:-------|:-----|
| 实现 SessionBar | SessionBar.tsx | 2d | - |
| 聚合会话统计 | AIChat.tsx | 1d | - |
| 添加折叠功能 | SessionBar.tsx | 1d | - |

### 5.3 Phase 3：分支功能（1-2 周）

| 任务 | 文件 | 工作量 | 依赖 |
|:-----|:-----|:-------|:-----|
| 修复 BranchIndicator | BranchIndicator.tsx | 1d | - |
| 实现分支选择对话框 | BranchSelectorDialog.tsx | 2d | - |
| 集成分支切换逻辑 | AIChat.tsx | 2d | SwitchBranch API |

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 6. 数据结构补充

### 6.1 成本计算字段

**现状**：`cost_estimate` 字段已存在但未填充

**修复方案**：在 `UpdateBlockStatus` 时计算成本

```go
// 后端：server/router/api/v1/ai/handler.go
func calculateCostEstimate(stats *SessionStats) int64 {
    if stats == nil || stats.TotalTokens == 0 {
        return 0
    }

    // DeepSeek V3 定价
    // Input: ¥0.14/M tokens = 0.14 milli-cents/token
    // Output: ¥0.28/M tokens = 0.28 milli-cents/token
    inputCost := stats.InputTokens * 0.14
    outputCost := stats.OutputTokens * 0.28

    return int64(inputCost + outputCost)
}
```

### 6.2 持续时间计算

**现状**：`created_ts` 和 `updated_ts` 已存在

**前端计算**：
```typescript
const duration = block.assistantTimestamp - block.created_ts;
const durationSeconds = duration / 1000;
const display = durationSeconds < 60
  ? `${durationSeconds.toFixed(1)}s`
  : `${(durationSeconds / 60).toFixed(1)}m`;
```

### 6.3 TokenUsage 显示

**现状**：`token_usage` JSONB 字段已存在但前端未使用

**修复**：在 BlockHeader 中读取并显示

```typescript
interface TokenUsage {
  promptTokens: number;
  completionTokens: number;
  totalTokens: number;
  cacheReadTokens: number;
  cacheWriteTokens: number;
}

// 在 UnifiedMessageBlock.tsx - BlockHeader
{block.tokenUsage && block.tokenUsage.totalTokens > 0 && (
  <Tooltip content={`${block.tokenUsage.promptTokens} input + ${block.tokenUsage.completionTokens} output`}>
    <span className="flex items-center gap-1 text-xs">
      🪙 {(block.tokenUsage.totalTokens / 1000).toFixed(1)}k
      {block.tokenUsage.cacheReadTokens > 0 && (
        <span className="text-green-500" title="Cache read">
          +{block.tokenUsage.cacheReadTokens}
        </span>
      )}
    </span>
  </Tooltip>
)}
```

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 7. 总结

### 7.1 关键发现

1. **后端实现完整**：Unified Block Model 的后端实现是完整的，数据已正确持久化
2. **前端部分缺失**：Block Header 的部分元素未实现，特别是编辑功能
3. **数据流正确**：`event_stream` → `extractToolCalls()` 数据流是正确的
4. **文档过时**：文档中的 ASCII 图与实际实现不一致

### 7.2 优先级建议

1. **P0 - 必须修复**：
   - 实现 Block 编辑功能
   - 修复 Token/Cost 统计显示

2. **P1 - 重要功能**：
   - 完善 BlockHeader 信息显示
   - 实现 SessionBar
   - 修复 BranchIndicator

3. **P2 - 优化改进**：
   - 文档更新
   - 移动端优化

### 7.3 长期规划

1. **分支管理**：完整的对话树可视化
2. **搜索过滤**：按分支、按时间、按 Token 使用量筛选
3. **导出功能**：导出会话为 Markdown/JSON
4. **成本控制**：会话预算告警

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

## 附录

### A. 相关文件清单

| 文件路径 | 用途 | 修改优先级 |
|:---------|:-----|:-----------|
| `docs/dev-guides/AI_CHAT_INTERFACE.md` | 界面架构文档 | P2 |
| `web/src/pages/AIChat.tsx` | AI Chat 主页面 | P1 |
| `web/src/components/AIChat/UnifiedMessageBlock.tsx` | Block 组件 | P0 |
| `web/src/components/AIChat/ChatHeader.tsx` | 聊天头部 | P3 |
| `web/src/components/AIChat/ChatInput.tsx` | 输入区域 | P3 |
| `web/src/components/AIChat/BlockEditDialog.tsx` | 编辑对话框（新建） | P0 |
| `web/src/components/AIChat/SessionBar.tsx` | 会话汇总栏（新建） | P1 |
| `web/src/components/AIChat/BranchSelectorDialog.tsx` | 分支选择器（新建） | P2 |
| `server/router/api/v1/ai/handler.go` | 聊天处理器 | P0 |
| `server/router/api/v1/ai/block_manager.go` | Block 管理器 | P0 |

### B. API 依赖清单

| API | 状态 | 说明 |
|:----|:----|:----|
| `ListBlocks` | ✅ | 获取会话 Blocks |
| `GetBlock` | ✅ | 获取单个 Block |
| `CreateBlock` | ✅ | 创建新 Block |
| `UpdateBlock` | ✅ | 更新 Block |
| `DeleteBlock` | ✅ | 删除 Block |
| `AppendEvent` | ✅ | 追加事件 |
| `AppendUserInput` | ✅ | 追加用户输入 |
| `ForkBlock` | ✅ | 创建分支 |
| `ListBlockBranches` | ✅ | 列出分支 |
| `SwitchBranch` | ✅ | 切换分支 |
| `DeleteBranch` | ✅ | 删除分支 |

### C. 测试清单

- [ ] Block 编辑功能：编辑用户输入 → 创建新分支 → 重新生成
- [ ] Token/Cost 显示：所有模式下正确显示
- [ ] BlockHeader 信息：分支路径、持续时间、成本
- [ ] SessionBar：会话统计聚合和折叠
- [ ] BranchIndicator：分支选择和切换
- [ ] 移动端：所有功能在移动端可用

---

## 8. P0 任务实施记录

### 8.1 已完成 (2026-02-06)

#### P0-A001: Block 编辑功能 ✅

**文件**: `web/src/components/AIChat/BlockEditDialog.tsx`

**实现内容**:
1. 创建 BlockEditDialog 组件，包含：
   - 原始消息显示
   - 编辑区域（Textarea）
   - 警告信息（编辑将创建新分支）
   - 确认/取消按钮
   - useBlockEditDialog Hook 管理对话框状态

2. 更新 `UnifiedMessageBlock.tsx`:
   - 添加 `onEdit` prop 到 UnifiedMessageBlockProps
   - 添加 `blockId` prop 到 UnifiedMessageBlockProps
   - 在 BlockFooter 中添加编辑按钮（Pencil 图标）
   - 编辑按钮在流式输出时禁用（`isStreaming` 状态）

3. 更新国际化 (en.json + zh-Hans.json):
   - `ai.unified_block.edit`: "Edit"
   - `ai.unified_block.edit_title`: "Edit User Input"
   - `ai.unified_block.edit_description`: "Edit your message to create a new branch and regenerate"
   - `ai.unified_block.original_message`: "Original Message"
   - `ai.unified_block.edited_message`: "Edited Message"
   - `ai.unified_block.edit_warning`: "Note: Editing will create a new branch..."
   - `ai.unified_block.cancel`: "Cancel"
   - `ai.unified_block.create_branch`: "Create Branch & Regenerate"
   - `ai.unified_block.branch_created`: "Branch created successfully"
   - `ai.unified_block.duration`: "Duration"
   - `ai.unified_block.cost`: "Cost"
   - `ai.unified_block.tokens`: "Tokens"
   - `ai.unified_block.branch_path`: "Branch"

**待集成**: ChatMessages.tsx 中集成 BlockEditDialog 并处理编辑确认逻辑

#### P0-A006: Normal 模式 Token/Cost 显示修复 ✅

**文件**: `web/src/components/AIChat/UnifiedMessageBlock.tsx`

**修改前**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary || (parrotId !== "GEEK" && parrotId !== "EVOLUTION")) return null;
  // ...
}, [blockSummary, parrotId]);
```

**修改后**:
```typescript
const geekSummary = useMemo(() => {
  if (!blockSummary) return null;  // 移除 parrotId 检查，所有模式都显示
  // ...
}, [blockSummary]);  // 移除 parrotId 依赖
```

**效果**: NORMAL、GEEK、EVOLUTION 所有模式都显示 Token/Cost 统计

#### P0-A007: ForkBlock API Hook ✅

**文件**: `web/src/hooks/useBlockQueries.ts`

**新增内容**:
```typescript
export function useForkBlock() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async ({ blockId, reason }: { blockId: bigint; reason?: string }) => {
      const response = await aiServiceClient.forkBlock({ id: blockId, reason });
      return response;
    },
    onSuccess: (newBlock, variables) => {
      // Add the new forked block to cache
      const conversationId = Number(newBlock.conversationId);
      queryClient.setQueryData(blockKeys.detail(Number(newBlock.id)), newBlock);

      // Invalidate the block list to show the new branch
      queryClient.invalidateQueries({
        queryKey: blockKeys.list(conversationId),
      });
    },
  });
}
```

### 8.2 待集成

1. **ChatMessages.tsx 集成**:
   - 导入 BlockEditDialog 和 useBlockEditDialog
   - 传递 onEdit 回调到 UnifiedMessageBlock
   - 处理编辑确认后调用 useForkBlock

2. **BranchIndicator 完善** (P1):
   - 显示当前分支路径（如 `A.1`、`B.2.3`）
   - 点击展开分支选择器

3. **SessionBar 实现** (P1):
   - 新建组件聚合会话统计
   - 显示在聊天界面顶部或侧边

---

**文档结束**
