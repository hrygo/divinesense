...
│   ├── review/          #   审查服务（间隔重复）
│   ├── context/         #   上下文构建
│   ├── runner/          #   🔴 CCRunner v2.0 (Hot-Multiplexing 核心执行引擎)
│   └── config.go        #   AI 配置
├── plugin/              # 其他可选插件（非 AI）
│   ├── scheduler/       # 任务调度
│   ├── storage/         # 存储适配器（S3、本地）
...
|   `GEEK`    | 极客 | —                              | Claude Code CLI 全双工 Hot-Multiplexing 执行  |
| `EVOLUTION` | 进化 | —                              | 自我进化（源代码修改，强制跨轮次持久化）      |
...
- 延迟：~0ms
- **Hot-Multiplexing 命中**: 针对 Geek/Evolution 模式，若 Session 存活，跳过所有路由。
...
**GeekMode 优先路由 (Multiplexing)**：
- 当 `GeekMode=true` 时，**绕过所有路由**，直接使用全局 `geekRunner` 单例进行 **Hot-Multiplexing** 挂载。
- **工作目录**: `~/.divinesense/claude/user_{id}`（用户沙箱）
- **会话持久化**: 基于 UUID v5 生成 SessionID，跨重启可热复用 OS 进程。
- **安全等级**: 中（沙箱隔离 + PGID 级生命周期销毁）
...
## CC Runner v2.0 异步架构 (Geek Mode 核心)

**规格文档**：[CC Runner 异步架构说明书](../architecture/cc-runner-architecture.md) (v2.0)

**概述**：Geek Mode 采用**全双工持久化热多路复用**架构。通过维护一个全局单例进程池，极大降低了 Claude Code CLI 的启动延迟。
...
│                     Backend (Go)                                │
│  ┌─────────────┐  ┌──────────────┐  ┌──────────────────────┐  │
│  │Session Pool │◄─┤  Multiplexer │◄─┤  PGID Controller     │  │
│  │ (Singleton) │  │  (Stdin/Out) │  │  (Graceful Shutdown) │  │
│  └─────────────┘  └──────────────┘  └──────────────────────┘  │
└──────────────────────────────┼──────────────────────────────────┘
                               │
┌──────────────────────────────▼──────────────────────────────────┐
...
| 组件                 | 文件                 | 描述                                          |
| :------------------- | :------------------- | :-------------------------------------------- |
| **CCSessionManager** | `session_manager.go` | 全局进程池管理（30min 闲置 GC，UUID v5 路由） |
| **CCRunner**         | `runner.go`          | 核心单例引擎（Hot-Multiplexing 调度）         |
| **DangerDetector**   | `danger_detector.go` | 危险命令防火墙（正则匹配拦截）                |
| **SessionStats**     | `session_stats.go`   | 实时流式指标结算（thinking, tokens, tools）   |
