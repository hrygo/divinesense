# Agent 架构设计模式

> **目标读者**: Agent 系统架构师、AI 工程师
> **核心内容**: 主流架构模式、工具使用模式、记忆系统

---

## 主流架构模式

### 模式对比

| 架构模式 | 核心思想 | 适用场景 | 优势 | 劣势 |
|:---------|:---------|:---------|:-----|:-----|
| **ReAct** | Reasoning + Acting 循环 | 多步推理任务 | 直观、易实现 | 迭代次数多时延迟高 |
| **Plan-and-Execute** | 先规划后执行 | 复杂多工具任务 | 执行高效、可并行 | 规划阶段额外开销 |
| **Reflexion** | 自我反思修正 | 需要高质量输出的任务 | 输出质量高 | 成本高、速度慢 |
| **Self-Consistency** | 多路径采样投票 | 不确定推理任务 | 提升准确率 | 计算成本倍增 |
| **Direct Calling** | 原生 Function Calling | 简单任务 (<3步) | 快速、低成本 | 不适合复杂任务 |

### 模式选择指南

```
任务复杂度判断：
│
├─ 简单 (< 3步)
│   └─ Direct Function Calling
│
├─ 中等 (3-10步)
│   └─ ReAct Loop
│
├─ 复杂多工具
│   └─ Plan-and-Execute
│
└─ 高质量需求
    └─ Reflexion + Self-Consistency
```

---

## ReAct 模式

### 结构

```markdown
Thought: [分析当前状态]
Action: [选择工具/行动]
Observation: [观察结果]
... (重复)
Thought: [得出结论]
Answer: [最终答案]
```

### 实现

```yaml
# ReAct 配置示例
strategy: react
max_iterations: 10

system_prompt: |
  You are a helpful assistant.

  ## Instructions
  Think step by step before taking action.
  After each action, observe the result before proceeding.

  ## Format
  Thought: ...
  Action: tool_name(input=...)
  Observation: ...
```

---

## Plan-and-Execute 模式

### 结构

```
┌─────────────────────────────────────┐
│           Phase 1: Planning          │
│  - 分析任务                          │
│  - 拆解子任务                        │
│  - 识别依赖关系                      │
│  - 生成执行计划                      │
└─────────────────┬───────────────────┘
                  │
┌─────────────────▼───────────────────┐
│        Phase 2: Execution           │
│  - 按计划执行                        │
│  - 并行执行独立任务                  │
│  - 处理执行结果                      │
└─────────────────────────────────────┘
```

### 实现

```yaml
# Plan-and-Execute 配置示例
strategy: planning
max_iterations: 15

system_prompt: |
  You are a planning expert.

  ## Phase 1: Planning
  1. Understand the user's goal
  2. Break down into sub-tasks
  3. Identify dependencies
  4. Create execution plan

  ## Phase 2: Execution
  1. Execute tasks according to plan
  2. Parallelize independent tasks
  3. Handle errors and adjust plan
```

---

## Reflexion 模式

### 结构

```
┌─────────────────────────────────────┐
│           Step 1: Initial           │
│           生成初步答案               │
└─────────────────┬───────────────────┘
                  │
┌─────────────────▼───────────────────┐
│         Step 2: Reflection          │
│  - 评估答案质量                      │
│  - 识别问题和改进空间                │
└─────────────────┬───────────────────┘
                  │
┌─────────────────▼───────────────────┐
│         Step 3: Refinement          │
│  - 根据反思结果改进答案              │
│  - 生成更优版本                      │
└─────────────────────────────────────┘
```

### 实现

```yaml
# Reflexion 配置示例
strategy: reflexion
max_iterations: 3
quality_threshold: 0.8

system_prompt: |
  You are a quality-focused assistant.

  ## Workflow
  1. Generate initial response
  2. Reflect on quality:
     - Is the response accurate?
     - Is it complete?
     - What could be improved?
  3. Refine based on reflection
  4. Repeat until quality threshold met
```

---

## 记忆系统架构

### 三层记忆模型

```
┌─────────────────────────────────────────────────────────┐
│                    短期记忆 (Short-term)                 │
│  • 容量: 4K-32K tokens (当前上下文窗口)                   │
│  • 存储: 内存中的消息历史                                 │
│  • TTL: 单次会话                                          │
└────────────────────┬────────────────────────────────────┘
                     │ 定期总结 + 压缩
┌────────────────────▼────────────────────────────────────┐
│                    情景记忆 (Episodic)                   │
│  • 容量: 无限 (可持久化)                                  │
│  • 存储: PostgreSQL / Redis                              │
│  • 索引: 向量 Embedding                                   │
│  • TTL: 30 天 (可配置)                                    │
└────────────────────┬────────────────────────────────────┘
                     │ 提取关键信息
┌────────────────────▼────────────────────────────────────┐
│                    长期记忆 (Long-term)                   │
│  • 容量: 无限                                             │
│  • 存储: 向量数据库 + RAG                                 │
│  • 索引: pgvector / Redis Vector                          │
│  • TTL: 永久                                              │
└─────────────────────────────────────────────────────────┘
```

### 记忆检索策略

| 策略 | 速度 | 质量 | 适用场景 |
|:-----|:-----|:-----|:---------|
| **滑动窗口** | 快 | 低 | 简单对话 |
| **语义检索** | 中 | 中 | 上下文相关查询 |
| **混合检索** | 慢 | 高 | 复杂查询 |
| **重排序** | 最慢 | 最高 | 精确答案需求 |

---

## 多 Agent 协作

### 协作模式

| 模式 | 描述 | 适用场景 |
|:-----|:-----|:---------|
| **顺序协作** | Agent 依次处理 | 流水线任务 |
| **并行协作** | Agent 同时处理 | 独立子任务 |
| **辩论协作** | Agent 相互辩论 | 决策优化 |
| **层级协作** | 主 Agent 协调子 Agent | 复杂系统 |

### 层级协作架构

```
┌─────────────────────────────────────────────────────────┐
│                    Coordinator Agent                    │
│  • 任务分解                                               │
│  • Agent 选择                                            │
│  • 结果综合                                               │
└────────────────────┬────────────────────────────────────┘
                     │
        ┌────────────┼────────────┐
        │            │            │
┌───────▼─────┐ ┌───▼────┐ ┌─────▼─────┐
│  Expert 1   │ │Expert 2│ │  Expert 3 │
│  (Domain A) │ │Domain B│ │  (Domain C)│
└─────────────┘ └────────┘ └───────────┘
```

---

## 提示词 vs 代码：边界划分

### 边界矩阵

| 职责 | 提示词占比 | 代码占比 | 说明 |
|:-----|:----------|:---------|:-----|
| **角色行为** | 80% | 20% | 主要靠提示词引导 |
| **工具调用** | 40% | 60% | 代码负责安全和验证 |
| **数据流** | 20% | 80% | 代码负责实现 |
| **记忆管理** | 30% | 70% | 代码负责存储和检索 |
| **错误处理** | 10% | 90% | 代码负责实现 |

### 原则

> **提示词用于**: 行为指导、灵活性、可解释性
> **代码用于**: 确定性逻辑、性能关键路径、安全性

---

## 常见陷阱与解决方案

### 1. 提示词过长

**问题**: 提示词占用大量上下文窗口

**解决方案**:
- 分层提示词：基础角色 + 动态上下文
- 使用结构化数据替代自然语言
- 惰性加载：仅在需要时注入详细说明

### 2. 指令冲突

**问题**: 不同模块的提示词指令互相冲突

**解决方案**:
- 建立指令优先级：工具层 > Agent 层 > 全局层
- 使用显式覆盖标记
- 在代码层进行冲突检测

### 3. 循环依赖

**问题**: Agent A 调用 Agent B，Agent B 又调用 Agent A

**解决方案**:
- 设计时绘制依赖图，检测环路
- 设置最大调用深度
- 使用超时机制

### 4. 提示词注入

**问题**: 用户输入恶意指令绕过安全检查

**解决方案**:
- Action-Selector Pattern: 受限的动作选择器
- Plan-Then-Execute Pattern: 先规划再执行
- 输入验证和过滤
- 沙箱化工具执行

---

## DivineSense 项目实践

### UniversalParrot 架构

```
UniversalParrot (配置驱动系统)
│
├─ ParrotFactory (工厂)
├─ ParrotConfig (配置加载)
│
├─ ExecutionStrategy (执行策略)
│   ├─ DirectExecutor (原生调用)
│   ├─ ReActExecutor (思考-行动循环)
│   └─ PlanningExecutor (两阶段规划)
│
└─ ToolRegistry (工具注册表)
    ├─ memo_search
    ├─ schedule_add
    ├─ schedule_query
    └─ find_free_time
```

### 配置示例

```yaml
# config/parrots/schedule.yaml
name: schedule
display_name: Schedule Parrot
emoji: "📅"
strategy: direct
max_iterations: 5

tools:
  - schedule_add
  - schedule_query
  - schedule_update
  - find_free_time

system_prompt: |
  You are a helpful assistant for managing schedules and calendars.
  ...
```

---

## 延伸阅读

- [AutoGen: Multi-Agent Conversation Framework](https://microsoft.github.io/autogen/)
- [MetaGPT: Multi-Agent Framework](https://github.com/geekan/MetaGPT)
- [Anthropic: Building Effective Agents](https://www.anthropic.com/engineering/building-effective-agents)
