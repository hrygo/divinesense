...
### SQLite（开发 + 客户端部署 - 完整 AI 支持）

- **状态**：开发环境 + 客户端部署
- **AI 功能**：**完整支持** —— 向量搜索、对话持久化、情景记忆、用户偏好
- **向量搜索**：应用层 cosine 相似度计算（存储 JSON 序列化向量）
- **全文搜索**：FTS5（可用）或 LIKE fallback
- **推荐用途**：本地开发、桌面客户端、离线部署
- **限制**：
  - 向量搜索性能不如 PostgreSQL + pgvector（应用层计算）
  - 大数据集（>10k 向量）性能下降
  - JSONB 替换为 TEXT 存储（丧失 JSON 路径查询）
- **维护状态**：积极维护
