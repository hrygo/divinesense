import { BellIcon, CalendarDays, EarthIcon, LibraryIcon, PaperclipIcon, Share2, SparklesIcon, Target, UserCircleIcon } from "lucide-react";
...
const Navigation = (props: Props) => {
  ...
  const scheduleNavLink: NavLinkItem = {
    id: "header-schedule",
    path: Routes.SCHEDULE,
    title: t("schedule.title"),
    icon: <CalendarDays className="w-6 h-auto shrink-0" />,
  };
  const reviewNavLink: NavLinkItem = {
    id: "header-review",
    path: Routes.REVIEW,
    title: t("review.title"),
    icon: <Target className="w-6 h-auto shrink-0" />,
  };
  const knowledgeGraphNavLink: NavLinkItem = {
    id: "header-knowledge-graph",
    path: Routes.KNOWLEDGE_GRAPH,
    title: t("ai.knowledge-graph.title") || "Graph",
    icon: <Share2 className="w-6 h-auto shrink-0" />,
  };
  ...
  const navLinks: NavLinkItem[] = currentUser
    ? [homeNavLink, chatNavLink, scheduleNavLink, reviewNavLink, knowledgeGraphNavLink, exploreNavLink, attachmentsNavLink, inboxNavLink]
  ...
  return (
    <header className={cn("w-full h-full overflow-auto flex flex-col justify-between items-start gap-4 hide-scrollbar", className)}>
      <nav
        aria-label={t("common.navigation") || "Main navigation"}
        className="w-full px-1 flex flex-col justify-start items-start space-y-2 overflow-auto overflow-x-hidden hide-scrollbar shrink"
      >
        <NavLink className="mb-1 cursor-pointer" to={currentUser ? Routes.CHAT : Routes.EXPLORE}>
        ...
        {navLinks.map((navLink) => (
          <NavLink
            className={({ isActive }) =>
              cn(
                "min-h-[44px] px-2 py-2 rounded-xl border flex flex-row items-center text-lg text-sidebar-foreground transition-colors cursor-pointer",
