...
>(({ className, children, side = "right", ...props }, ref) => {
  return (
    <SheetPortal>
      ...
      >
        {children}
        <SheetPrimitive.Close className="ring-offset-background data-[state=open]:bg-secondary absolute top-4 right-4 rounded-sm opacity-60 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none">
...
const SheetHeader = React.forwardRef<React.ElementRef<"div">, React.ComponentPropsWithoutRef<"div">>(({ className, ...props }, ref) => {
  return <div ref={ref} data-slot="sheet-header" className={cn("flex flex-col gap-1.5 px-4 pt-2 pb-4", className)} {...props} />;
});
