...
| 代理  | 名称     | 类型 | 擅长 | 示例 |
|:---:|:-------|:---:|:-------|:-----|
| 🦜 | **灰灰** | 常规 | 知识检索 | "我写过关于 React 的什么内容？" |
| 🦜 | **金刚** | 常规 | 日程管理 | "帮我安排明天下午3点的会议" |
| 🦜 | **惊奇** | 常规 | 综合助理 | "总结我这周的工作和待办" |
| 🦜 | **极客** | 特殊 | 代码助手 | "帮我重构这个函数"（面向用户） |
| 🧬 | **进化** | 特殊 | 自我进化 | "优化数据库查询性能"（面向系统） |

**特殊模式说明**：
- **极客模式**：用户开启后，AI 帮你写代码、调试、分析技术问题
- **进化模式**：管理员开启后，AI 修改 DivineSense 自身代码实现进化 |

### 📚 研究与最佳实践

- **[极客模式：CLI Agent 最佳实践](docs/research/BEST_PRACTICE_CLI_AGENT.md)** - 深入探讨如何优化类似 Claude Code 的 CLI 代理，实现更高效的自动化编码流程。
- **[进化模式：自我进化能力](docs/specs/META_COGNITION.md)** - 了解 DivineSense 如何修改自身源代码实现自我进化。
...
| 特性 | Geek Mode (极客模式) | Evolution Mode (进化模式) |
|:-----|:---------------------|:-------------------------|
| **图标** | 🦜 | 🧬 |
| **目标用户** | 所有用户 | 仅管理员 |
| **用途** | 帮你处理代码任务 | 让系统自我进化 |
| **工作方式** | 调用 Claude Code CLI | 修改 DivineSense 源码 |
| **示例** | "帮我重构这个函数" | "优化数据库查询性能" |
| **安全机制** | 标准 AI 对话 | 严格约束 + PR 审查 |

**Geek Mode（极客模式）**：
- **面向用户**：像有一个 AI 编程助手帮你写代码
- 零 LLM 延迟，直接调用 Claude Code CLI
- 适用于：代码重构、调试、技术分析、生成单元测试

**Evolution Mode（进化模式）🧬**：
- **面向系统**：DivineSense 修改自己的源代码实现进化
- 允许系统读取、理解、修改自身代码
- 严格安全约束 + 强制 PR 审查 + 仅限管理员
- 适用于：性能优化、Bug 修复、功能增强
