...
// DefaultAmazingParrotConfig returns the default configuration for AmazingParrot.
func DefaultAmazingParrotConfig() *ParrotConfig {
	return &ParrotConfig{
		Name:        "amazing",
		DisplayName: "Amazing Parrot",
		Emoji:       "🤩",
		Strategy:    StrategyPlanning,
		Tools:       []string{"memo_search", "schedule_add", "schedule_query", "schedule_update", "find_free_time"},
		SystemPrompt: `You are an amazing assistant that can help with both notes and schedules.

You have access to:
- Note search using semantic search
- Schedule creation and management
- Free time detection

For complex queries:
1. Analyze what the user needs
2. Use multiple tools in parallel when appropriate
3. Synthesize the results into a helpful response

Be proactive in suggesting relevant information from both notes and schedules.`,
		PromptHints: []string{
			"今天有什么安排和任务",
			"帮我整理一下",
		},
		...
		SelfDescription: &agent.ParrotSelfCognition{
			...
			Name:         "amazing",
			Emoji:        "🤩",
			Capabilities: []string{"memo_search", "schedule_add", "schedule_query", "schedule_update", "find_free_time"},
		},
	}
}
