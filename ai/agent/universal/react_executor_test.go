...
// TestReActExecutor_ParseToolCall tests the parseToolCall function.
func TestReActExecutor_ParseToolCall(t *testing.T) {
	tests := []struct {
		name              string
		response          string
	...
	}{
		{
			...
			response:          "This is a simple answer",
			expectedToolName:  "",
			expectedToolInput: "",
			expectedCleanText: "This is a simple answer",
		},
		{
			name:              "tool call with TOOL prefix",
			response:          "TOOL: memo_search\nINPUT: {\"query\":\"test\"}",
			expectedToolName:  "memo_search",
			expectedToolInput: "{\"query\":\"test\"}",
			expectedCleanText: "",
		},
		{
			name:              "tool call with Tool prefix (lowercase t)",
			response:          "Tool: schedule_query\nInput: 2026-01-01",
			expectedToolName:  "schedule_query",
			expectedToolInput: "2026-01-01",
			expectedCleanText: "",
		},
		{
			name:              "tool call with clean text before",
			response:          "Let me search for that.\nTOOL: memo_search\nINPUT: test",
			expectedToolName:  "memo_search",
			expectedToolInput: "test",
			expectedCleanText: "Let me search for that.",
		},
		{
			name:              "tool call with mixed case",
			response:          "I'll help you find that.\n\nTOOL: memo_search\nINPUT: {\"query\":\"hello\"}\n\nDone.",
			expectedToolName:  "memo_search",
			expectedToolInput: "{\"query\":\"hello\"}",
			expectedCleanText: "I'll help you find that.",
		},
		{
			name:              "tool call with colon in input",
			response:          "TOOL: schedule_query\nINPUT: 2026-01-01 to 2026-01-02",
			expectedToolName:  "schedule_query",
			expectedToolInput: "2026-01-01 to 2026-01-02",
			expectedCleanText: "",
		},
		{
			name:              "empty response",
		...
		{
			name:              "multiline clean text",
			response:          "I will search for you.\nPlease wait.\nTOOL: memo_search\nINPUT: test",
			expectedToolName:  "memo_search",
			expectedToolInput: "test",
			expectedCleanText: "I will search for you. Please wait.",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			toolName, toolInput, cleanText := parseToolCall(tt.response)

			if toolName != tt.expectedToolName {
				t.Errorf("toolName = %q, want %q", toolName, tt.expectedToolName)
			}
			if toolInput != tt.expectedToolInput {
				t.Errorf("toolInput = %q, want %q", toolInput, tt.expectedToolInput)
			}
			if cleanText != tt.expectedCleanText {
				t.Errorf("cleanText = %q, want %q", cleanText, tt.expectedCleanText)
			}
		})
	}
}

// TestReActExecutor_ParseToolCall_Truncation tests long clean text truncation.
func TestReActExecutor_ParseToolCall_Truncation(t *testing.T) {
	// Create a response with clean text longer than 200 runes
	longText := strings.Repeat("word ", 100) // 500 words = ~2000 chars
	response := fmt.Sprintf("%s\nTOOL: memo_search\nINPUT: test", longText)

	toolName, toolInput, cleanText := parseToolCall(response)

	if toolName != "memo_search" {
		t.Errorf("toolName = %q, want 'memo_search'", toolName)
	}
	if toolInput != "test" {
		t.Errorf("toolInput = %q, want 'test'", toolInput)
	}

	// Check that clean text is truncated
	runeCount := len([]rune(cleanText))
	if runeCount > 204 { // 200 + "..."
		t.Errorf("cleanText rune count = %d, want <= 204", runeCount)
	}
	if !strings.HasSuffix(cleanText, "...") {
		t.Error("cleanText should end with '...' when truncated")
	}
}
